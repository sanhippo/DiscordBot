...
async def extracommands(ctx, client):
    ...
    if ctx.content == ("$Supplyupdater"):
        if not (await check_cred(ctx, "Developer")):
            return 1
        await SupplyUpdater(ctx, client)
        return 1
...
async def SupplyUpdater(ctx, client):
    global running
...
async def do_on_message(message, client):

    if message.content.startswith("$"):
        message.content = message.content[:1] + message.content[1].upper() + message.content[2:]
        ctx1 = message

        if await extracommands(ctx1, client):
            return

        if message.content.startswith("$Test"):
            ctx1 = message

            if not (await check_cred(ctx1, "Developer")):
                return

            await message.channel.send("Please Select From Options:")
            try:
                reply = await client.wait_for('message', timeout=30, check=lambda m: auth_and_chan(ctx1)(m))
